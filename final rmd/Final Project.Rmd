...
\begin{itemize}
  ...
  \item breast height diameter of trees (measured at the height of an adult's breast, about 1.35m from ground)
  \item tree species
  \item health status
\end{itemize}
...
This graph is interesting and we will discuss it in the excutive summary section.
...
ggplot(df_tree_health) +
  ...
  guides(fill=guide_legend(reverse=TRUE)) + th
```
...
df_tree_top <- df_tree_health %>% 
  mutate(spc_common = ifelse(spc_common %in% 
                             top_five, spc_common, "Other")) %>%
...
\subsection{Links to codes}
The github repo can be found \href{https://github.com/hao871563506/EDAV-project--NYC-trees}{here}. The useful codes are in the final rmb file which can be found \href{https://github.com/hao871563506/EDAV-project--NYC-trees/tree/master/final%20rmd}{here}. The codes for the interactive part can be found \href{https://github.com/hao871563506/EDAV-project--NYC-trees/blob/master/final%20rmd/app.R}{here}.
...
\begin{itemize}
  \item Density of trees in NYC
  \item Growth of top five (as of 2015) species from 1995 to 2015
  \item Different health problems of trees
\end{itemize}
We will present the graphs and discuss our findings in the following pages (all graphs are color-blind friendly).
...
zip_choropleth(density_tree,
               zip_zoom = nyc_fips,
               num_colors = 1,
               title      = "Density of trees (per square mile) in NYC",
               legend     = "Tree per sqmi") + 
  scale_fill_gradient(low = "white", high = "#009E73") + 
  theme(plot.title = element_text(size=16))
```

It can be seen from the spatial heat map that the higher percentage of trees mostly fall in the area around central park while less percentages of trees lie in Midtown Manhattan and even more less fall in areas in Staten Island. 
...
In year 2015, the top five tree species in New York city are London planetree, honeylocust, Callery pear, pin oak and Norway maple. Each of five species presented a different growing trend in the past ten year. For example, the growing trend of London planetree is very stable. 

The most interesting is Morway maple. Knowing that the Norway maple is very invasive, the NYC government pulled seedlings of Norway maple from moist soil before they get too large to prevent them from destroying native ecosystems, causing trouble in yards and gardens, and creating visual blight. Therefore, the number of Norway maple decreased dramatically in the past ten years, varying from the first to the last. 


```{r, echo = FALSE, fig_width = 10}
boroughrootstone<-mydata %>% group_by(borough) %>% count(root_stone) %>% 
  mutate(proportion = n / sum(n))
boroughrootstone2<- subset(boroughrootstone, 
                           boroughrootstone$root_stone == "Yes")
...
Stone indicates the presence of a root problem caused by paving stones in tree bed, grate indicates the problem caused by metal grates in tree bed, wire indicates the problem caused by wires or rope wrapped around the trunk, light indicates problem caused by lighting installed on the tree and shoe indicates the presence of a branch problem caused by sneakers in the branches.
...
Our interactive component is done using shiny and can be found \href{https://ds3717.shinyapps.io/interactive/}{here}. 

There are two tabs in the Interactive component. The first one is the bar plot of species count, and the second is the density curve of the diameter of tree. Users may be interested in the most common species that they live in, and it is impossible to display with static graphs because there are too many zipcodes. Users can select year from select box and type in zipcode in text area. If the users type in invalid zipcode, it will alert the user. 

As to the species count bar plot, given a zipcode, we can vividly see the change of numbers of different species in different years, and we can directly compare the number between different species. As to the density curve of the tree diameter, one trend we want the users to discover is that the tree dbh distribution is shifting to the right which confirms the fact that trees are growing thicker. Besides, we can compare the density curve in a certain zipcode with that of all data.


\section{Conclusion}
After completing the project, we learn and practice our skills to clean, visualize and understand data. Especially, we learn from plotting density curve and species count for each zip code that if there is too much graphs to be plotted, using an interactive plot is a good idea. Otherwise, in our case, it is not practical to plot these 300 plots all in the R markdown file. 

Besides, we discovered the pros and cons for each type of graph, and learned to decide which graphs we should use. For example, we use a line graph to plot the changing of number of the species throughout these 20 years, and the line graph gives us a clear view of the changes better than for example, bar plot. 

Also, we found out that Github is a useful platform for groups work. When small changes happen, do a pull and push is a more convenient way than upload and download. But we did not utilize its power. Most of the time we just simply do downloads and uploads, and we will try to make more use of Github next time. This is a very meaningful project, we learn a lot from this experience.
